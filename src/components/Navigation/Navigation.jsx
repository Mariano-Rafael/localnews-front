...
function Navigation() {
    ...
    const handleLoginSuccess = (username) => { // Recebe o username como argumento
        setIsLoggedIn(true);
    ...
    return (
        <nav className="bg-gray-100 w-full p-2 h-11">
            <div className="container mx-auto flex justify-center">
                ...
                <div className="absolute right-4 flex space-x-2 items-center"> {/* Adicionado items-center aqui */}
                    {isLoggedIn && loggedInUsername ? (
                        <>
                            <span className="text-black mr-2">Bem vindo, {loggedInUsername}</span>
                            <button className="bg-black hover:bg-white hover:text-black text-white px-2 h-7 border border-black shadow-2xl transition duration-700 ease-in-ou" onClick={handleLogout}>Sair</button>
                        </>
                    ) : (
                        <>
                            <button
